...
- ✅ AuthMe-style premium bypass for verified Mojang accounts on offline-mode servers, without requiring a client mod
...
- 🔑 基于机器 ID + IP 的自动登录
- ✅ 离线服支持 AuthMe 风格正版免密：复用原版加密登录握手，无需客户端安装 Mod
...
TAuth 会在离线服中对已启用正版免密或正在 `/premium` 待验证的账号复用原版 `EncryptionRequest`/`EncryptionResponse` 登录握手，并通过 Mojang `hasJoined` 验证连接方确实拥有该正版账号。玩家需要先正常 `/login`，再执行 `/premium` 并使用正版启动器重连完成验证；验证成功后写入 `premiumUuid`，之后同一正版账号可跳过 `/login`。普通离线/盗版账号不会收到正版握手，仍走密码登录。

如果客户端也安装了 TAuth（`premiumAutoProofEnabled=true`，默认启），客户端会自动检测当前是否为 Minecraft 正版会话并上报。服务端通过 nonce + Mojang `hasJoined` 独立验证后，自动完成正版身份绑定，无需手动 `/premium`。客户端未安装或验证失败时，回退正常密码登录。
