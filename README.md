...
- ✅ Optional Mojang session proof when the client also installs TAuth, allowing verified premium sessions on offline-mode servers without TrueUUID
...
premiumSessionProofEnabled = true # Let TAuth clients prove Mojang sessions on offline-mode servers
```
...
### Premium Session Proof

When `premiumSessionProofEnabled = true` and the client also has TAuth installed, the server sends a nonce during the auth prompt. The client uses its local Mojang session token to call `joinServer`, then the server verifies the nonce with Mojang `hasJoined`. The token never leaves the client. If proof succeeds, TAuth treats this connection as a premium session for registration/login policy; if proof fails or the client is missing, normal password authentication still works.
...
- 🔑 基于机器 ID + IP 的自动登录
- ✅ 客户端也安装 TAuth 时可选进行 Mojang 会话证明，让离线服在不接 TrueUUID 的情况下识别正版会话
...
`premiumSessionProofEnabled = true` 时，如果客户端也安装 TAuth，服务端会发送随机 nonce，客户端用本地 Mojang token 调用 `joinServer`，服务端再用 Mojang `hasJoined` 校验。token 不会离开客户端；校验失败或客户端未安装时仍回退普通密码登录。
